--!strict
-- Test runner for CharacterSystem with improved mock implementation
...
function Instance.new(className: string, name: string)
    local self = {
        ClassName = className,
        Name = name or "",
        _children = {},
...
-- Instance methods
function InstanceMt:FindFirstChild(name: string, recursive: boolean?): any
...
function InstanceMt:FindFirstChildOfClass(className: string): any
    if not self._children then return nil end
...
function InstanceMt:IsA(className: string): boolean
    return self.ClassName == className
end

function InstanceMt:GetChildren(): {any}
    return self._children or {}
end

function InstanceMt:GetFullName(): string
...
local function createTestCharacter(className: string, health: number): any
    -- Create player
    local player = Instance.new("Model", className .. "Player")
    
    -- Add PlayerClass
    local playerClass = Instance.new("StringValue", "PlayerClass")
    playerClass.Value = className
    playerClass.Parent = player
    
    -- Create character
    local character = Instance.new("Model", className .. "Character")
    ...
    local playerRef = Instance.new("ObjectValue", "Player")
...
local invalidCharacter = createTestCharacter("InvalidClass", 100)
